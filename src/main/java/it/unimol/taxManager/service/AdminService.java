...
public class AdminService {
    ...
    public AdminService(GestioneUtentiClient gestioneUtentiClient,
                        SogliaRepository sogliaRepository,
                        StudentRepository studentRepository, JWTToken tokenManager, TaxRepository taxRepository, PagoPAClient pagoPAClient, ReportAndAnalysisClient reportAndAnalysisClient) {
    ...
    public boolean updateSoglieIsee(String token, int anno, UpdateBracketsDTO updatedSoglieDTO) {
        ...
        if (updatedSoglieDTO.importoBase() == null && updatedSoglieDTO.soglia1() == null && updatedSoglieDTO.sconto1() == null &&
                updatedSoglieDTO.soglia2() == null && updatedSoglieDTO.sconto2() == null && updatedSoglieDTO.soglia3() == null &&
                updatedSoglieDTO.sconto3() == null && updatedSoglieDTO.soglia4() == null && updatedSoglieDTO.sconto4() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Nessun valore da aggiornare fornito.");
        }

        BracketsDTO mergedSoglie = new BracketsDTO(
                sogliaToMerge.getAnno(),
                updatedSoglieDTO.importoBase() != null ? updatedSoglieDTO.importoBase() : sogliaToMerge.getImportoBase(),
                updatedSoglieDTO.soglia1() != null ? updatedSoglieDTO.soglia1() : sogliaToMerge.getSoglia1(),
                updatedSoglieDTO.sconto1() != null ? updatedSoglieDTO.sconto1() : sogliaToMerge.getSconto1(),
                updatedSoglieDTO.soglia2() != null ? updatedSoglieDTO.soglia2() : sogliaToMerge.getSoglia2(),
                updatedSoglieDTO.sconto2() != null ? updatedSoglieDTO.sconto2() : sogliaToMerge.getSconto2(),
                updatedSoglieDTO.soglia3() != null ? updatedSoglieDTO.soglia3() : sogliaToMerge.getSoglia3(),
                updatedSoglieDTO.sconto3() != null ? updatedSoglieDTO.sconto3() : sogliaToMerge.getSconto3(),
                updatedSoglieDTO.soglia4() != null ? updatedSoglieDTO.soglia4() : sogliaToMerge.getSoglia4(),
                updatedSoglieDTO.sconto4() != null ? updatedSoglieDTO.sconto4() : sogliaToMerge.getSconto4()
        );
        ...
        Brackets sogliaToSave = new Brackets(
                mergedSoglie.anno(),
                mergedSoglie.importoBase(),
                mergedSoglie.soglia1(),
                mergedSoglie.sconto1(),
                mergedSoglie.soglia2(),
                mergedSoglie.sconto2(),
                mergedSoglie.soglia3(),
                mergedSoglie.sconto3(),
                mergedSoglie.soglia4(),
                mergedSoglie.sconto4()
        );
    ...
    public Map<String, Object> generatePaymentNotices(String token, int anno) {
        int MAX_NUM_RATE = 5; // Numero massimo di rate da calcolare per ogni studente
        validateToken(token);
        ...
        for (Student studente : studenti) {
            ...
            if (studente.getStato() != StudentStatus.CESSATO && studente.getStato() != StudentStatus.COMPLETATO) {
                ...
                if (reportAndAnalysisClient.getStudentProgress(token, studente.getId()).progressPercentage() == 100) {
                    ...
                    if (importoStudente > 0.00 && taxRepository.findTaxesByStudent_Id(studente.getId()).stream().noneMatch(tax -> tax.getExpirationDate().isAfter(LocalDate.of(LocalDate.now().getYear(), 6, 1)))) {
                        // Calcolo delle rate
                        int numeroRate = MAX_NUM_RATE;
                        ...
                        for (int i = 0; i < numeroRate; i++) {
                            PagoPATaxDTO pagoPaDetails = new PagoPATaxDTO(
                                    studentDetails.id(),
                                    studentDetails.surname(),
                                    studentDetails.name(),
                                    "Università degli studi del Molise",
                                    importoPerRata,
                                    getExpiration(i + 1, anno));
    ...
    private void checkSoglie(BracketsDTO sogliaToCheck) {
        if (sogliaToCheck.importoBase() < 0 || sogliaToCheck.soglia1() < 0 || sogliaToCheck.sconto1() < 0 ||
                sogliaToCheck.soglia2() < 0 || sogliaToCheck.sconto2() < 0 || sogliaToCheck.soglia3() < 0 || sogliaToCheck.sconto3() < 0 ||
                sogliaToCheck.soglia4() < 0 || sogliaToCheck.sconto4() < 0) {
